...
std::string neopixel_init(int max_leds) {
    ...
    std::string err = spi_open(NEOPIXEL_SPI_DEV, NEOPIXEL_SPI_HZ, NEOPIXEL_SPI_MODE, s_spi_fd);
...
std::string neopixel_set_rgb(const uint8_t (*rgb)[3], int count) {
    if (s_spi_fd < 0) return "neopixel_set_rgb: not initialized";
    ...
    std::string err = spi_write(s_spi_fd, buf.data(), (int)buf.size());
...
std::string neopixel_set_rgbw(const uint8_t (*rgbw)[4], int count) {
    if (s_spi_fd < 0) return "neopixel_set_rgbw: not initialized";
...
std::string neopixel_clear(int count) {
    if (s_spi_fd < 0) return "neopixel_clear: not initialized";
    if (count < 1 || count > s_max_leds)

#include "bmp390.h"
#include "i2c.h"
#include "../vendored/BMP3_SensorAPI/bmp3.h"

#include <unistd.h>
#include <cstring>

// ─── Bosch API callbacks ────────────────────────────────────────

static int s_i2c_fd = -1;

static BMP3_INTF_RET_TYPE bmp3_i2c_read(uint8_t reg_addr, uint8_t* reg_data,
                                          uint32_t len, void* intf_ptr) {
    (void)intf_ptr;
    std::string err = i2c_read_reg_buf(s_i2c_fd, reg_addr, reg_data, (int)len);
    return err.empty() ? BMP3_OK : BMP3_E_COMM_FAIL;
}

static BMP3_INTF_RET_TYPE bmp3_i2c_write(uint8_t reg_addr, const uint8_t* reg_data,
                                           uint32_t len, void* intf_ptr) {
    (void)intf_ptr;
    std::string err = i2c_write_reg_buf(s_i2c_fd, reg_addr, reg_data, (int)len);
    return err.empty() ? BMP3_OK : BMP3_E_COMM_FAIL;
}

static void bmp3_delay_us(uint32_t period, void* intf_ptr) {
    (void)intf_ptr;
    usleep(period);
}

// ─── State ──────────────────────────────────────────────────────

static struct bmp3_dev s_dev;
static struct bmp3_settings s_settings;
static bool s_bmp390_ok = false;

// ─── Public API ─────────────────────────────────────────────────

std::string bmp390_init(int i2c_fd) {
    s_bmp390_ok = false;
    s_i2c_fd = i2c_fd;

    std::string err = i2c_set_slave(i2c_fd, 0x76);
    if (!err.empty()) return "bmp390_init: " + err;

    memset(&s_dev, 0, sizeof(s_dev));
    memset(&s_settings, 0, sizeof(s_settings));

    s_dev.intf = BMP3_I2C_INTF;
    s_dev.read = bmp3_i2c_read;
    s_dev.write = bmp3_i2c_write;
    s_dev.delay_us = bmp3_delay_us;
    s_dev.intf_ptr = nullptr;

    int8_t rc = bmp3_init(&s_dev);
    if (rc != BMP3_OK)
        return "bmp390_init: bmp3_init failed, rc=" + std::to_string(rc);

    uint32_t settings_sel = BMP3_SEL_PRESS_EN | BMP3_SEL_TEMP_EN |
                            BMP3_SEL_PRESS_OS | BMP3_SEL_TEMP_OS |
                            BMP3_SEL_ODR;

    s_settings.press_en = BMP3_ENABLE;
    s_settings.temp_en = BMP3_ENABLE;
    s_settings.odr_filter.press_os = BMP3_OVERSAMPLING_8X;
    s_settings.odr_filter.temp_os = BMP3_OVERSAMPLING_2X;
    s_settings.odr_filter.odr = BMP3_ODR_50_HZ;

    rc = bmp3_set_sensor_settings(settings_sel, &s_settings, &s_dev);
    if (rc != BMP3_OK)
        return "bmp390_init: bmp3_set_sensor_settings failed, rc=" + std::to_string(rc);

    s_settings.op_mode = BMP3_MODE_NORMAL;
    rc = bmp3_set_op_mode(&s_settings, &s_dev);
    if (rc != BMP3_OK)
        return "bmp390_init: bmp3_set_op_mode failed, rc=" + std::to_string(rc);

    s_bmp390_ok = true;
    return "";
}

std::string bmp390_read(int i2c_fd, float& pressure_kpa, float& temperature_c) {
    pressure_kpa = 0.0f;
    temperature_c = 0.0f;
    if (!s_bmp390_ok) return "bmp390_read: sensor not initialized";

    s_i2c_fd = i2c_fd;
    std::string err = i2c_set_slave(i2c_fd, 0x76);
    if (!err.empty()) return "bmp390_read: " + err;

    struct bmp3_data data;
    int8_t rc = bmp3_get_sensor_data(BMP3_PRESS_TEMP, &data, &s_dev);
    if (rc != BMP3_OK)
        return "bmp390_read: bmp3_get_sensor_data failed, rc=" + std::to_string(rc);

    pressure_kpa = (float)(data.pressure / 1000.0);
    temperature_c = (float)data.temperature;
    return "";
}
